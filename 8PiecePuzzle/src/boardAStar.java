...
public class boardAStar{
	...
	static HashMap<Integer, Integer> rowPositionInOriginal = new HashMap<>();
		static HashMap<Integer, Integer> colPositionInOriginal = new HashMap<>();
	...
	public static void main(String[] args) throws Exception {
		...
		for(int r=1;r<n;r++){
			...
			for(int c=0;c<n;c++){
				if(stringarray[c].equals(".")){finalArray[r][c] = 0; rowPositionInOriginal.put(0, r); colPositionInOriginal.put(0, c);}
					else 
						finalArray[r][c] = Integer.parseInt(stringarray[c]); 
						rowPositionInOriginal.put(finalArray[r][c], r);
						colPositionInOriginal.put(finalArray[r][c], c);
			}
		}
		...
		String goal = Astar(finalArray, s);
	...
	public static int g(int[][] b){
		...
		for(int i=0;i<b.length;i++){
			for(int j=0;j<b.length;j++){
				int k = rowPositionInOriginal.get(b[i][j]);
				int l = colPositionInOriginal.get(b[i][j]);
				c+=Math.abs((b[i][j]-k)+(b[i][j]-l));
			}
		}
		return c;
	}

	public static String Astar(int[][] a, String path){
